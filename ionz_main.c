...
int main(int argc, char **argv) {
  ...
  alpha_H_dt = (double)alpha_H *(double)input_param.mass_unit * (double)Msun2kg / (double)hmass * dt;
  alpha_H_dt /= (double)pow3((double)Mpc2cm *(double)(input_param.gridsize/input_param.Hubble_h));

  if(mympi.ThisTask == 0)
    printf("alpha_H * dt = %lg, dt = %lg years",alpha_H_dt,dt/(3600*24*365.25));
  ...
  for(ii=0;ii<NjobsperTask[mympi.ThisTask];ii++) { 
    if(use_prev_xfrac == 1)
      reionization_with_xfrac(Radii_list[JobsTask[ii]], nh, ngamma, xfrac, nxion, nion, Nnion, N1, N2, N3);
    else
